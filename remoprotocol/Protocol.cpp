...
bool ProtocolStruct::decode(const char *const msgconst, int msglen) {
    if (msglen < protocolHeaderLength) { // decode时,接收到的消息长度可能不够,这种情况不能由本地避免,因此返回错误而不是直接退出
        LOG(INFO) << "ProtocolStruct::decode error msglen = " << msglen << ", protocolHeaderLength = "
...
CommProtoVariables::MSGinfo CommProtoVariables::gen_respond_status(CommDeviceEnum target, uint16_t packseq) {
    ...
    status = htonl(status);
    return gen_request_respond(target, COMMCMDSET_UNIVERSAL, COMMCMDID_DEVICE_STATUS, true, false, packseq, RESPOND,
                               (char *) &status, sizeof(status));
}

CommProtoVariables::MSGinfo CommProtoVariables::gen_request_version(CommDeviceEnum target) {
    return gen_request_respond(target, COMMCMDSET_UNIVERSAL, COMMCMDID_DEVICE_VERSION, true, false, packSeq_++,
                               REQUEST);
}
...
CommProtoVariables::MSGinfo CommProtoVariables::gen_respond_version(CommDeviceEnum target, uint16_t packseq) {
    ...
    self.firmware = ntohl(0);
    self.updateStatus = 0;
    return gen_request_respond(target, COMMCMDSET_UNIVERSAL, COMMCMDID_DEVICE_VERSION, true, false, packseq, RESPOND,
                               (char *) &self, sizeof(self));
}

bool CommProtoVariables::sync_do_request(MSGinfo msginfo, int timeout) {
    return CommAsyncUDP::Get()->sync_send_msg(ENDPOINT_CAMERA, msginfo.msgptr, timeout); //无重发
}
