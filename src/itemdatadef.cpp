...
void init_itemData()
{
    ...
    itemData.insert(constructItemData_Camera_CmdId_Set_Required_(Remo_CmdId_Camera_Set_CapOperation));
...
//                            {{Aperture_FNO_1 , "FNO.1"}, {Aperture_FNO_1_4 , "FNO.1.4"}, {Aperture_FNO_2, "FNO.2"}, {Aperture_FNO_2_8, "FNO.2.8"}, {Aperture_FNO_4, "FNO.4"}}));
    ...
    itemData.insert(constructItemData_Camera_(
                           Remo_CmdId_Camera_Bott, Remo_CmdId_Camera_Bott, Remo_CmdId_Camera_Get_Current_Shutter_Range, "当前快门范围",
                            {{Shutter_1_DividedBy_8000, "1/8000"}, {Shutter_1_DividedBy_6400 , "1/6400"}, {Shutter_1_DividedBy_5000, "1/5000"},
                             {Shutter_1_DividedBy_4000, "1/4000"}, {Shutter_1_DividedBy_3200 , "1/3200"}, {Shutter_1_DividedBy_2500, "1/2500"},
                             {Shutter_1_DividedBy_2000, "1/2000"}, {Shutter_1_DividedBy_1600 , "1/1600"}, {Shutter_1_DividedBy_1250, "1/1250"},
                             {Shutter_1_DividedBy_1000, "1/1000"}, {Shutter_1_DividedBy_800 , "1/800"}, {Shutter_1_DividedBy_640, "1/640"},
...
//                             {ISO_6400 , "关闭"}, {ISO_12800 , "3张合成"}, {ISO_25600, "5张合成"}}));
    itemData.insert(constructItemData_Camera_(
                           Remo_CmdId_Camera_Bott, Remo_CmdId_Camera_Bott, Remo_CmdId_Camera_Get_Current_ISO_Range, "当前ISO范围",
                            {{ISO_100 , "关闭"}, {ISO_200 , "3张合成"}, {ISO_400, "5张合成"},
                             {ISO_800 , "关闭"}, {ISO_1600 , "3张合成"}, {ISO_3200, "5张合成"},
                             {ISO_6400 , "关闭"}, {ISO_12800 , "3张合成"}, {ISO_25600, "5张合成"}}));
    itemData.insert(constructItemData_Camera_(
                           Remo_CmdId_Camera_Bott, Remo_CmdId_Camera_Bott, Remo_CmdId_Camera_Get_Current_Aperture_Range, "当前光圈范围",
                            {{Aperture_FNO_1 , "FNO.1"}, {Aperture_FNO_1_4 , "FNO.1.4"}, {Aperture_FNO_2, "FNO.2"}, {Aperture_FNO_2_8, "FNO.2.8"}, {Aperture_FNO_4, "FNO.4"}}));
    itemData.insert(constructItemData_Camera_(Remo_CmdId_Camera_Bott, Remo_CmdId_Camera_Bott, Remo_CmdId_Camera_Get_Current_Evbias, "当前曝光补偿范围",
                            {{Evbias_Neg_3 , "-3.0"}, {Evbias_Neg_2_7 , "-2.7"}, {Evbias_Neg_2_3, "-2.3"},
                             {Evbias_Neg_2 , "-2.0"}, {Evbias_Neg_1_7 , "-1.7"}, {Evbias_Neg_1_3, "-1.3"},
                             {Evbias_Neg_1 , "-1.0"}, {Evbias_Neg_0_7 , "-0.7"}, {Evbias_Neg_0_3, "-0.3"},
                             {Evbias_0 , "0"}, {Evbias_0_3 , "0.3"}, {Evbias_0_7, "0.7"},
...
void addItem2Map(void * item, Remo_CmdId_Camera_e idGet, int idSet, int idRange, int set, bool addItemData)
{
    ...
    if (addItemData) {
        auto it = itemData.find({set, idGet, idSet, idRange});
        if (it == itemData.end()) {
            bool findRange = false;
            std::set<SubItemData> subData;
            for (auto it : itemData) {
                if (set == it.CmdSet && idRange == it.CmdId_GetRange) {
                    findRange = true;
                    subData = it.subItemData;
                }
            }
            if (findRange) {
                itemData.insert({set, idGet, idSet, idRange, "", subData});
            }
            else {
                itemData.insert({set, idGet, idSet, idRange});
            }
            return;
        }
        else {
...
void *findUiPtrById(Remo_CmdId_Camera_e idGet, Remo_CmdId_Camera_e idSet, Remo_CmdId_Camera_e idGetRange, Remo_CmdSet_e set)
{
    for (auto it : ui2ItemData) {
        ItemData item = it.second;
        if (set == item.CmdSet &&
                (idGet == item.CmdId_GetData && idSet == item.CmdId_SetData && idGetRange == item.CmdId_GetRange)) {
            return it.first;
        }
    }
    return nullptr;
